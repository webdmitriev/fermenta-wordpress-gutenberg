      ...
      <div class="line-wrap df-fs-st w-100p">
        <?php if( have_rows('programs') ) : while ( have_rows('programs') ) : the_row(); ?>
          <div class="services-item <?= get_sub_field('btn_text') ? '' : 'without-btn'; ?>" style="background-color: <?= $isDark ? '#dacdc2' : '#b2c4ad'; ?>">
            ...
            <?php if(get_sub_field('btn_text')): ?><a href="<?= esc_url(get_permalink(get_sub_field('btn_link'))); ?>" class="btn btn-gray" style="color: <?= $isDark ? '#493a2f' : '#becdc2'; ?>;background-color: <?= $isDark ? '#b0a195' : '#6f7e73'; ?>"><?php echo wp_kses(get_sub_field('btn_text'), $allowed_tags); ?></a><?php endif; ?>
          </div>
        <?php endwhile; endif; ?>
      </div>
    </div>
