 ...
 */
  ...
  // icon, link, rule (link, email, phone)
  $social_footer  = get_field('social_media');
  $social_menu    = get_field('social_media_menu');
...
?>
  ...
  <footer class="footer">
    <div class="container">
      ...
      <?php if( have_rows('social_media', 'options') ) : while ( have_rows('social_media', 'options') ) : the_row();
        ...
        $get_rule = get_sub_field('rule');
        $rule  = $rules[strtolower($get_rule)] ?? '';
      ?>
        <a href="<?php echo $rule; ?><?php echo $link; ?>" class="social-item" target="_blank" rel="noopener noreferrer"><img src="<?php echo $icon; ?>" alt="icon" /></a>
      <?php endwhile; endif; ?>
    </div>
  </footer>
  ...
  <div class="social-media">
    ...
    <?php if( have_rows('social_media_menu', 'options') ) : while ( have_rows('social_media_menu', 'options') ) : the_row();
      $icon = esc_url(get_sub_field('icon'));
      $link = esc_url(get_sub_field('link'));
      $get_rule = get_sub_field('rule');
      $rule  = $rules[strtolower($get_rule)] ?? '';
